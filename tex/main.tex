...
One idea is to utilize the equivalence relations used to define the monoid. Can
we sort them and then use as a source for name building?
...
Let $\N$ be the additive monoid of natural numbers, let $S = {0, 1, 2, ...,
11}$, and let $\cdot: \N \times S \to S$ be the action given in Example
3.1.2.3. Using a nice small generating set for the monoid, write out the
corresponding action table.
...
For any $m \in \N$ let $i_m : \N \to \Z$ be the function $i_m(n) = m*n$. All
such functions are monoid homomorphisms $(\N,0,+) \to (\Z,0,+)$. Do any monoid homo-
morphisms $(\N, 0, +) \to (\Z, 0, +)$ not come in this way? For example, what about using
$n \mapsto 5*n + 1$ or $n \mapsto n^2$ , or some other function?
