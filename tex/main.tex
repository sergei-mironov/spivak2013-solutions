...
(a) Let $X$ and $Y$ be sets.. construct the "swap map" $s:(X,Y)\to(Y,X)$
\ans
$s:(X,Y)\to(Y,X) = (,)\circ\langle\pi_2,\pi_1\rangle$
...
Note: $(f:X \to Y)$ is an isomorphism if $\exists (g:Y \to X): g \circ f = id_X \land f \circ g = id_Y$

Note: diagram $(f,g,h)$ commutes if $f \circ g = h$
\ans

In universal property of products, put $A$ equal to $Y * X$ and get $\exists !
g:(Y*X)\to(X,Y)$. In a similar way, we have $\exists! s:(X,Y)\to(Y,X)$.
We need to show that $g \circ s = id_{(X*Y)}$ and $s \circ g = id_{(Y*X)}$.
