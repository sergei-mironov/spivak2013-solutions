...
\begin{tikzcd}
  \set{((p_1,p_2),\Delta)|p_1,p_2 \in \text{people}, \Delta \in \mathbb{R}} \arrow[r, "\pi_2"] \arrow[d, "\pi_1"] & \set{\Delta|-0.5 < \Delta < 0.5} \arrow[d, "is"] \\
  \fbox{\text{pair of people}} \arrow[r, "h_2 - h_1"]           & \mathbb{R}
\end{tikzcd}
\end{center}
...
\begin{tikzcd}
  ...
  \set{(\fbox{The Virgin Mary},p_2)|p_2 \in \text{people}} \ar[r, "is"] & \fbox{\text{pair of people}} \arrow[r, "h_2 - h_1"]           & \mathbb{R}
\end{tikzcd}
\end{center}
...
\text{Authors} \arrow[r, "\text{num of papers}", "\text{const}_1"', Rightarrow] & \mathbb{R}
\end{tikzcd}
\end{center}

where $const_1 : \text{authors} \to \mathbb{R}$ maps all authors to $1$.
...
Consider the set R of real numbers. Draw the coordinate plane $\mathbb{R}
\times \mathbb{R}$, give it coordinates $x$ and $y$. A binary relation on
$\mathbb{R}$ is a subset $S \subseteq \mathbb{R} \times \mathbb{R}$, which can
...
(d) Consider the equivalence relation S1 generated by $\set{(1,2),(1,3)}$.
...
visualizations in $\R \times \mathbb{R}$; what are they?
